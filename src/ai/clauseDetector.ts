import { DocumentAnalyzer, DocumentAnalysis } from './documentAnalyzer';
import { ServiceRegistry } from '../core/serviceRegistry';
import { Logger } from '../core/logger';
import { EventBus } from '../core/eventBus';

// Types for clause detection
...
export class ClauseDetector {
    ...
    constructor() {
        ...
        this.documentAnalyzer = ServiceRegistry.getInstance().get<DocumentAnalyzer>('documentAnalyzer');
    }
    ...
    async initialize(): Promise<void> {
        ...
        // Register for events
        this.eventBus.subscribe<DocumentAnalysis>('document.analyzed', {
            id: 'clause-detector-analyzed',
            handle: this.processAnalyzedDocument.bind(this)
        });
        
        this.eventBus.subscribe<string>('clause.detect', {
     ...
     */
    private async loadClausePatterns(): Promise<void> {
        try {
            ...
            throw new Error(`Clause pattern loading failed: ${(error as Error).message}`);
        }
    }
     ...
     */
    private createIndemnificationPattern(): ClausePattern {
        return {
     ...
     */
    private createLimitationOfLiabilityPattern(): ClausePattern {
        return {
     ...
     */
    private createConfidentialityPattern(): ClausePattern {
        return {
     ...
     */
    private createTerminationPattern(): ClausePattern {
        return {
            id: 'termination-001',
     ...
     */
    private createGoverningLawPattern(): ClausePattern {
        return {
     ...
     */
    private createForceMatjeurePattern(): ClausePattern {
        return {
     ...
     */
    private createDisputeResolutionPattern(): ClausePattern {
        return {
     ...
     */
    private createAssignmentPattern(): ClausePattern {
        return {
            id: 'assignment-001',
     ...
     */
    private createIntellectualPropertyPattern(): ClausePattern {
        return {
     ...
     */
    private createNonCompetePattern(): ClausePattern {
        return {
            id: 'non-compete-001',
     ...
     */
    async processAnalyzedDocument(analysis: DocumentAnalysis): Promise<void> {
        try {
     ...
     */
    async detectClauses(document: string): Promise<ClauseDetectionResult> {
        ...
        try {
            ...
            // Initialize result
            const result: ClauseDetectionResult = {
                clauses: [],
            ...
            throw new Error(`Clause detection failed: ${(error as Error).message}`);
        }
    }
     ...
     */
    private async detectClausesWithPatterns(document: string): Promise<DetectedClause[]> {
        ...
        for (const pattern of sortedPatterns) {
            ...
            const containsKeyPhrase = pattern.keyPhrases.some(phrase => 
                document.toLowerCase().includes(phrase.toLowerCase())
            );
            ...
            while ((match = regex.exec(document)) !== null) {
                ...
                const keyPhraseCount = pattern.keyPhrases.filter(phrase => 
    ...
    private async classifyClauses(
        ...
        for (const clause of clauses) {
            ...
            if (documentAnalysis) {
                const overlappingClauses = documentAnalysis.structure.clauses.filter(
                    analysisClause => {
                        // Check if the clauses overlap
                        return (
                            (clause.startPosition <= analysisClause.endPosition &&
                ...
                if (overlappingClauses.length > 0) {
                    ...
                    const mostImportantClause = overlappingClauses.reduce(
                        (prev, curr) => (curr.importance > prev.importance ? curr : prev),
                        overlappingClauses[0]
                    );
                    
                    // Update the classification
                    clause.classification.type = mostImportantClause.type;
                    clause.classification.confidence = Math.min(
                        clause.classification.confidence + 0.15,
                        0.98
                    );
     ...
     */
    private getPossibleAlternativeTypes(type: string): string[] {
        const allTypes = [
            'INDEMNIFICATION',
