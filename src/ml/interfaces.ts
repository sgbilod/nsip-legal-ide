...
export interface PredictionFactor {
    name: string;
    influence: number; // -1.0 to 1.0 where negative values hurt the case
    description: string;
    evidence: string[];
}
...
export interface CompoundRisk {
    name: string;
    components: string[];
    amplification: number; // 1.0+ (how much stronger the combined risk is)
    description: string;
...
export interface RiskMitigation {
    ...
    actions: string[];
    costEstimate: number;
    effectivenessEstimate: number; // 0.0 to 1.0
    timeframe: string;
...
export interface CompliancePrediction {
    currentCompliance: number; // 0.0 to 1.0
    predictedCompliance: number; // 0.0 to 1.0
